...
typedef struct {
    unsigned int errTest:1;
    unsigned int errInvalidMethod:1;
}server_Errors;
...
#define __raiseErr(err)                     do{ __errors.err = 1; CONFIG_raiseError(server); }while(0)
...
//== HTTP METHOD
#define __httpMethod                        (server_self.httpMethod)
#define __setHttpMethod(meth)               do{ __httpMethod = (meth); }while(0)
